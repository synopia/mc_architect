 ...
 * @author synopia
 */
public class SliceRenderer implements Renderer<Slice, SliceRenderContext> {
    @Override
    public void render(Slice slice, SliceRenderContext c) {
        ...
        for( int y = sy; y < ey; y++ ) {
            for( int x = sx; x < ex; x++ ) {
                ...
                if( colorForBlock!=null ) {
                    ...
                    Point2i curr = c.worldToScreen(position.toPoint2d());
