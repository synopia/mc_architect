...
//        logger.info(String.format("%d, %d, %d -> %d, %d, %d", chunkStartX, startY, chunkStartZ, chunkEndX, endY, chunkEndZ));
            ...
            for( int chunkZ=chunkStartZ; chunkZ<=chunkEndZ; chunkZ++ ) {
                ...
                if( chunk!=null ) {
                    ...
                    if( imageCache.containsKey(chunk.getId()) ) {
                        ...
                        image = new BufferedImage(16, sliceType==SliceType.Y?16:128,BufferedImage.TYPE_INT_ARGB);
    ...
    protected void drawImage(RenderContext c, Image image, Chunk chunk, Position pos, SliceType sliceType, int startY, int endY ) {
        int sx;
        int sy;
        int ex;
        int ey;
